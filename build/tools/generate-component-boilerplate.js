    ...
    return {
        ...
        handler: async ({ name, type = "ui", features = ["inputs", "outputs", "accessibility", "docs"], includeReadme = true, includeTests = false }) => {
            try {
                ...
                const patterns = await analyzeExistingPatterns(type);
                // Generate component template
                const template = generateComponentTemplate(name, type, features, patterns);
                // Format output
 ...
 */
async function analyzeExistingPatterns(type) {
    const patterns = {
        imports: new Set(),
        baseClasses: new Set(),
        commonInputs: new Set(),
        commonOutputs: new Set(),
        designPatterns: new Set()
    };
    try {
 ...
 */
function extractImportPatterns(content, patterns) {
    const importLines = content.match(/import\s+{[^}]+}\s+from\s+['"][^'"]+['"];/g) || [];
    importLines.forEach(line => {
        if (line.includes('@angular/core'))
            patterns.imports.add('@angular/core');
        if (line.includes('@angular/common'))
            patterns.imports.add('@angular/common');
        if (line.includes('lucide-angular'))
            patterns.imports.add('lucide-angular');
        if (line.includes('computed'))
            patterns.imports.add('computed');
        if (line.includes('signal'))
            patterns.imports.add('signal');
    });
}
/**
 * Extract common class patterns
 */
function extractClassPatterns(content, patterns) {
    if (content.includes('readonly'))
        patterns.baseClasses.add('readonly-signals');
    if (content.includes('computed('))
        patterns.baseClasses.add('computed-properties');
    if (content.includes('inject('))
        patterns.baseClasses.add('modern-di');
}
/**
 * Extract input/output patterns
 */
function extractInputOutputPatterns(content, patterns) {
 ...
 */
function generateComponentTemplate(name, type, features, patterns) {
 ...
 */
function generateTypeScriptFile(componentName, selector, type, features, patterns) {
 ...
 */
function generateHtmlFile(componentName, type, features) {
    const hasInputs = features.includes('inputs');
    const hasVariants = features.includes('variants');
    const hasLoading = features.includes('loading');
 ...
 * Generate SCSS file
 */
function generateScssFile(componentName, type, features) {
 ...
 */
function generateReadmeFile(componentName, selector, type, features) {
    const hasInputs = features.includes('inputs');
    const hasOutputs = features.includes('outputs');
    const hasVariants = features.includes('variants');
 ...
 */
function generateSpecFile(componentName, type, features) {
 ...
 */
function formatComponentBoilerplate(template, includeReadme, includeTests) {
...
//# sourceMappingURL=generate-component-boilerplate.js.map