...
import { 
  findComponentFiles, 
  findServiceFiles, 
  getAverageTemplateComplexity,
  getModelDescription 
} from '../utils/analysis-helper.js';
...
export function createAnalyzeArchitectureTool() {
  return {
    ...
    handler: async ({ focusArea }: AnalysisOptions): Promise<ToolResponse> => {
      try {
        let analysis = '';
 ...
 */
async function analyzeComponents(): Promise<string> {
 ...
 */
async function analyzeServices(): Promise<string> {
 ...
 */
async function analyzeRouting(): Promise<string> {
  try {
 ...
 * Analyze Data Models
 */
async function analyzeDataModels(): Promise<string> {
  try {
 ...
 * Analyze All Aspects
 */
async function analyzeAll(): Promise<string> {
